...
struct FHQ_Treap_词频压缩 {
    struct Node {
        int val = 0;            // 节点存储的值
        int cnt = 0;            // 当前值的出现次数（多重集支持）
    ...
    void split1(int cur, int rank, int &l, int &r) {  // 按照rank分裂
        ...
        // 左 ： 小于等于rank的元素
        // 右 ： 大于rank的元素
        if (node[node[cur].l].size + node[cur].cnt <= rank) {  // 当前节点应归到左子树
            l = cur;
            split(node[cur].r, rank - node[node[cur].l].size - node[cur].cnt, node[cur].r, r);
            update(l);
        } else {  // 当前节点应归到右子树
            r = cur;
    ...
    // 修改节点 val 的计数，delta 可以为正（增加）或负（减少）
    void change_cnt(int cur, int val, int delta) {
        if (node[cur].val == val) {
            node[cur].cnt += delta;
        } else if (node[cur].val > val) {
            change_cnt(node[cur].l, val, delta);
        } else {
            change_cnt(node[cur].r, val, delta);
        }
        update(cur);
    }
    ...
    void insert(int val) {
        if (find(root, val)) {
            // 如果存在该值，计数加1
            change_cnt(root, val, 1);
        } else {
            // 不存在该值，新建节点并合并
            int l, r;
            split(root, val, l, r);  // 分裂成 <= val 和 > val 两棵树
            node[++cnt].val = val;
            node[cnt].cnt = node[cnt].size = 1;
    ...
    // 删除值 val
    void erase(int val) {
        int cur = find(root, val);
        if (cur == 0) return;  // 不存在，直接返回
        if (node[cur].cnt > 1) {
            // 计数大于1，计数减1
            change_cnt(root, val, -1);
        } else {
            ...
            root = merge(l, r);        // 删除lm对应节点
        }
    }

    // 计算子树 cur 中小于 val 的元素个数（含计数）
    int cnt_less_than(int cur, int val) {
        if (cur == 0) return 0;
        if (node[cur].val >= val)
            return cnt_less_than(node[cur].l, val);
        else
            return node[node[cur].l].size + node[cur].cnt + cnt_less_than(node[cur].r, val);
    }

    // 返回 val 的排名（小于 val 的元素个数 + 1）
    int order_of_val(int val) { return cnt_less_than(root, val) + 1; }
    ...
    int find_by_order(int k) {
        ...
        while (cur) {
            ...
            } else if (k > l_size + node[cur].cnt) {// k 在右子树中
                k -= l_size + node[cur].cnt;
                cur = node[cur].r;
            } else {
                return node[cur].val;
            }
